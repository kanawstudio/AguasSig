...
var services = {
	...
	"wms_TOPO":{
		...
		layers:[
			{name:"Topográfico",url:"https://idecan3.grafcan.es/ServicioWMS/MTI?",layers:"WMS_MTI",format:"image/png"},
...
// Servicios : (un único string separado por comas)
//    "wms_OU" Ortofoto Urbana
//    "wms_OE" Ortoexpress
//    "wms_CA"  Callejero
//    "wms_MIX" Ortofoto+Callejero
//    "wms_TOPO" Mapa Topográfico 1:5.000 (junto con Modelo Digital del Terreno)
// [Optional] : Lat,Long,Zoom -> Coordenadas del centro y nivel de zoom
//    Por defecto : todo canarias
// [Optional] : info -> Habilitar pedir información haciendo click sobre el mapa (GetFeatureInfo)
...
// Funcion para añadir un WMS de usuario (se creará como un overlay sobre la capa base actual).
...
function IDECanarias_AgregarWMS(nombre,urlwms,capas,formato,transparencia,activar){
	...
	//Añadimos boton para incluir capa externa
...
// Funcion para añadir el logotipo de IDECanarias
...
// Función para crear una caja de búsqueda en IDECanarias
...
// Carga un WMS dados sus parámetros (url, capas,...)
// Los parámetros opacity,maxZoom y WMS_BGColor son opcionales
...
function ResultadoBusqueda(cadena){
	...
	if (jsonArray.length > 0){
		...
		divHTML += " VALUE = '-9' disabled='disabled'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Resultados búsqueda en IDECanarias</OPTION>"
...
function ConstruirHtml(coord,desc,clas,local,dist,image,nombre,i){
	...
	if (desc != ""){
		contentString += "<label>";
		contentString +="<font style='font-size:10px;font-weight:bold;color:#666666'><b>Descripción:</b>&nbsp;</font><font style='font-size: 9pt; color: #AAAAAA'>"+desc+"</font>";
		contentString += "</label>";
		contentString +="<br/>";
	}
	...
	if (clas != ""){
		contentString += "<label>";
		contentString +="<font style='font-size: 10px;font-weight:bold;color:#666666'><b>Clasificación:</b>&nbsp;</font><font style='font-size: 9pt; color: #AAAAAA'>"+clas+"</font>";
		contentString += "</label>";
		contentString +="<br/>";
	}
	...
	if (local != ""){
		contentString += "<label>";
		contentString +="<font style='font-size:10px;font-weight:bold;color:#666666'><b>Localización:</b>&nbsp;</font><font style='font-size: 9pt; color: #AAAAAA'>"+local+"</font>";
		contentString += "</label>";
		contentString +="<br/>";
	}
